...
input [64 +1:0] dma2bpt_req_pd; /* pkt_id_width=1 pkt_widths=78,PRIMARY_MEMIF_WIDTH+2  */
...
wire [64 +1:0] ipipe_pd;
wire [64 +1:0] ipipe_pd_p;
wire ipipe_rdy;
wire ipipe_rdy_p;
wire ipipe_vld;
wire ipipe_vld_p;
...
NV_NVDLA_NOCIF_DRAM_WRITE_IG_BPT_pipe_p1 pipe_p1 (
  ...
  ,.dma2bpt_req_pd (dma2bpt_req_pd[64 +1:0]) //|< i
  ,.dma2bpt_req_valid (dma2bpt_req_valid) //|< i
  ,.ipipe_rdy_p (ipipe_rdy_p) //|< w
  ,.dma2bpt_req_ready (dma2bpt_req_ready) //|> o
  ,.ipipe_pd_p (ipipe_pd_p[64 +1:0]) //|> w
...
NV_NVDLA_NOCIF_DRAM_WRITE_IG_BPT_pipe_p2 pipe_p2 (
  ...
  ,.ipipe_vld_p (ipipe_vld_p) //|< w
  ,.ipipe_pd (ipipe_pd[64 +1:0]) //|> w
  ,.ipipe_rdy_p (ipipe_rdy_p) //|> w
  ,.ipipe_vld (ipipe_vld) //|> w
  );
assign ipipe_cmd_vld = ipipe_vld && (ipipe_pd[64 +1:64 +1]== 0 /* PKT_nvdla_dma_wr_req_dma_write_cmd_int_ID  */ );
assign dfifo_wr_vld = ipipe_vld && (ipipe_pd[64 +1:64 +1]== 1 /* PKT_nvdla_dma_wr_req_dma_write_data_int_ID  */ );
//:my $k = 1;
//:if ($k == 1) {
//: print qq(assign dfifo_wr_mask = 2'b11;\n);
//:}
//:else {
//: print qq(assign dfifo_wr_mask = ipipe_pd[64 +1 -1:64];\n);
//:}
...
// **************************************************************************************************************
// Generated by ::pipe -m -bc -os ipipe_pd_p (ipipe_vld_p,ipipe_rdy_p) <= dma2bpt_req_pd[64 +1:0] (dma2bpt_req_valid,dma2bpt_req_ready)
// **************************************************************************************************************
...
input [64 +1:0] dma2bpt_req_pd;
input dma2bpt_req_valid;
input ipipe_rdy_p;
output dma2bpt_req_ready;
output [64 +1:0] ipipe_pd_p;
output ipipe_vld_p;
reg dma2bpt_req_ready;
reg [64 +1:0] ipipe_pd_p;
reg ipipe_vld_p;
reg [64 +1:0] p1_pipe_data;
reg [64 +1:0] p1_pipe_rand_data;
reg p1_pipe_rand_ready;
reg p1_pipe_rand_valid;
reg p1_pipe_ready;
reg p1_pipe_ready_bc;
reg [64 +1:0] p1_pipe_skid_data;
reg p1_pipe_skid_ready;
reg p1_pipe_skid_valid;
reg p1_pipe_valid;
reg p1_skid_catch;
reg [64 +1:0] p1_skid_data;
reg p1_skid_ready;
...
always @(
  ...
  p1_pipe_rand_data = dma2bpt_req_pd[64 +1:0];
  `else
// VCS coverage off
  ...
  p1_pipe_rand_data = (p1_pipe_rand_active)? 'bx : dma2bpt_req_pd[64 +1:0];
// VCS coverage on
  `endif
end
`ifndef SYNTHESIS
...
// **************************************************************************************************************
// Generated by ::pipe -m -bc -is ipipe_pd (ipipe_vld,ipipe_rdy) <= ipipe_pd_p[64 +1:0] (ipipe_vld_p,ipipe_rdy_p)
// **************************************************************************************************************
...
input nvdla_core_clk;
input nvdla_core_rstn;
input [64 +1:0] ipipe_pd_p;
input ipipe_rdy;
input ipipe_vld_p;
output [64 +1:0] ipipe_pd;
output ipipe_rdy_p;
output ipipe_vld;
reg [64 +1:0] ipipe_pd;
reg ipipe_rdy_p;
